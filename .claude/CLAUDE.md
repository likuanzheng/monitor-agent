...
│   └── gpu.json            # 各设备 GPU exporter 地址(:32021) + device 标签
├── exporters/              # 设备侧 exporter 部署文件（跑在被监控机上，非本机服务）
│   ├── A100/               # 天数机：node_exporter+cadvisor+ix-exporter 的 compose
│   └── B100/node_exporter/ # Jetson(Orin)：compose + 自研 jetson_exporter(gpu_* 指标)
...
- **跨平台 GPU 统一**：天数离散卡(ix-exporter `ix_*`)与 Jetson(自研 jetson_exporter `gpu_*`)
  指标模型对齐(同 label/后缀/单位、仅前缀不同)，`config.yaml` 里每个 GPU query 用 `ix_X or gpu_X`
  并集同时命中两类设备，engine 无需改。加新厂商 GPU exporter 同理：并集里再 `or 前缀_X`。
...
当前纳管 **两台**设备，三 target 均 `up`：
- **A100-infer-01（192.168.32.153）**：天数卡，GPU 经 `ix_*` 已验证正常聚合。
- **B100-01（192.168.32.210）**：实为 NVIDIA Jetson AGX Orin（"B100"是自研板内部型号名），GPU 走 `gpu_*`。

Agent 已验证：温度/内存/每核使用率/磁盘/docker 容器/GPU 均正常；GPU 用 `ix_X or gpu_X` 并集跨平台统一。
`cpu_freq_mhz` 仍 null（设备侧未出频率指标）。
⚠️ **未完成的部署**（详见 PROGRESS.md 待办）：① B100 上要重部署新版 jetson_exporter（旧版无 util/显存，
Orin GPU 才会出现）；② 重建 monitor-agent 加载新 config。B100 容器数为 0 待查（cgroup 正则）。
